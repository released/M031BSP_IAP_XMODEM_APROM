...
void SystemReboot_CHIP_RST(void)     // Reset I/O and peripherals ,  BS(FMC_ISPCTL[1]) reload from CONFIG setting (CBS)
{
...
static uint16_t CalCheckSum(uint32_t start, uint32_t len)
{
    ...
    {
        ReadData(start + i, start + i + FMC_FLASH_PAGE_SIZE, (unsigned int *)aprom_buf);
